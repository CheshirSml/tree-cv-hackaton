  ...
  export default defineComponent({
    ...
    methods: {
          ...
          if (files && files.length) {
            ...
            await $api.post(this.api, formData, headers)
              .then((response : any) => {
                ...
                this.$emit('uploaded', selectedFile.name)
              })
              .catch((error : any) => {
      ...
      removePhoto (url: string) {
        ...
        // TODO: объекты надо переносить
        const urls = this.urls.slice(0)
        console.log('urls', urls)
        const ids = this.ids.slice(0)
        const index = urls.findIndex((x: any) => x == url)
        console.log('index', index, url)
        if (index == -1) {
          return
        }
        const id = ids[index]
        this.processingPhotoUrl = url
        $api.delete(this.api + id + '/')
          .then((response : any) => {
            urls.splice(index, 1)
            ids.splice(index, 1)
            this.$emit('update:ids', ids)
            this.$emit('update:urls', urls)
            this.processingPhotoUrl = null
          ...
          .catch((error : any) => {
            this.processingPhotoUrl = null
            console.log('FAILURE!!', error)
            toast.error("Не удалось удалить фото. " + errors.getErrorText(error), {
              autoClose: 8000,
            })
          })
...
<template>
  <div class="d-flex flex-column">
    ...
    <div class="d-flex">
      <div class="inline-block position-relative" v-for="photoItem in displayPhotoUrls">
        <VBtn icon="ri-close-fill"
          style="top: -10px; position: absolute; right: 10px; z-index: 1;"
          color="error"
          @click="removePhoto(photoItem)"
          :disabled="!!processingPhotoUrl"
          :loading="processingPhotoUrl == photoItem"
        ></VBtn>
        <VAvatar
          rounded="lg"
          size="100"
    ...
    <div class="d-flex flex-column justify-center gap-5 mt-4">
